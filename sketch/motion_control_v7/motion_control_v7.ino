...
void loop() {
  ...
  if(ch3<1300){
    ...
    digitalWrite(modePin, HIGH);  //manual operation by RC
  }
  else{
  //input from ROS publisher, when ch3 at stable position, input taken from turtlebot_teleop
    ...
    turn=turnROS;
    digitalWrite(modePin, LOW); //autonomous operation
  }
...
void unicycleRun(int forward, int turn){
  ...
  Ur=right_calib*(forward-turn/2)/10; //here turning in cw is positive
  Ul=left_calib*(forward+turn/2)/10;
