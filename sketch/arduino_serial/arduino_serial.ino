...
void changeVar(const geometry_msgs::Twist& input){
  forwardROS=map(input.linear.x, -500, 500, 0, 255);
  turnROS=map(input.angular.z, -500, 500, 0, 255);

  //updates calibration factor
  right_calib=(int)input.linear.y;
  left_calib=(int)input.linear.z; 
}
...
void loop() {
  ...
  Wire.write(forwardROS);
  Wire.write(turnROS);
  Wire.write(right_calib);
  Wire.write(left_calib);
